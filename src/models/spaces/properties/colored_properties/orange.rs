...
impl OrangeProperty {
    pub fn buy_property(&mut self, player_ref: PlayerRef) {
        ...
        match self {
            ...
            | OrangeProperty::NewYorkAve { state } => *state = PropertyState::Owned,
        }
        player.add_property(Properties::ColoredProperty(ColoredProperties::Orange(
            *self,
        )));
    }
    ...
    pub fn auction(&mut self, board: &Board) {
        ...
        loop {
            ...
            if round_bids.iter().all(|&b| b) {
                ...
                winner.add_property(Properties::ColoredProperty(ColoredProperties::Orange(
                    *self,
                )));
                println!(
                    "Player {:?} has aquired {:?} for ${:?}",
                    winner.player_number,
                    ColoredProperties::Orange(*self),
                    bid_price - 10
                );
                break;
            }
    ...
    pub fn buy_house(&mut self, player_ref: PlayerRef, board: &Board) {
        ...
        let new_state = match self {
            OrangeProperty::StJamesPlace { state }
            | OrangeProperty::TennesseeAve { state }
            | OrangeProperty::NewYorkAve { state } => match state {
    ...
    pub fn update_property_state(
        ...
        if let Some(property) = player.properties.iter_mut().find(|p| match p {
            Properties::ColoredProperty(ColoredProperties::Orange(inner)) => inner == self,
            _ => false,
        }) {
            if let Properties::ColoredProperty(ColoredProperties::Orange(inner)) = property {
                match inner {
                    OrangeProperty::StJamesPlace { state }
                    | OrangeProperty::TennesseeAve { state }
                    | OrangeProperty::NewYorkAve { state } => {
    ...
    pub fn legal_house_purchase(&self, board: &Board) -> bool {
        let (st_james_place, tennessee_ave, new_york_ave) = self.house_count(board);
        if vec![st_james_place, tennessee_ave, new_york_ave]
            .windows(2)
            .all(|window| (window[0] as i8 - window[1] as i8).abs() <= 1)
        {
            match self {
                OrangeProperty::StJamesPlace { state } => {
                    st_james_place > tennessee_ave && st_james_place > new_york_ave
                }
                OrangeProperty::TennesseeAve { state } => {
                    tennessee_ave > st_james_place && tennessee_ave > new_york_ave
                }
                OrangeProperty::NewYorkAve { state } => {
                    new_york_ave > st_james_place && new_york_ave > tennessee_ave
                }
            }
        } else {
            false
    ...
    pub fn house_count(&self, board: &Board) -> (u8, u8, u8) {
        if self.has_monopoly(board) {
            let st_james_place = match self {
                OrangeProperty::StJamesPlace { state } => match state {
                    PropertyState::Houses(house_count) => match house_count {
                        HouseCount::Zero => 0,
                        HouseCount::One => 1,
                        HouseCount::Two => 2,
            ...
            let tennessee_ave = match self {
                OrangeProperty::TennesseeAve { state } => match state {
                    PropertyState::Houses(house_count) => match house_count {
                        HouseCount::Zero => 0,
                        HouseCount::One => 1,
                        HouseCount::Two => 2,
            ...
            (st_james_place, tennessee_ave, new_york_ave)
        } else {
            (0, 0, 0)
        }
    }
    pub fn has_monopoly(&self, board: &Board) -> bool {
        if let Some(owner_ref) = self.get_owner(board) {
            ...
            let monopoly = vec![
                Properties::ColoredProperty(ColoredProperties::Orange(
            ...
            monopoly.iter().all(|property| {
                if owner.properties.contains(property) {
                    match property {
                        Properties::ColoredProperty(ColoredProperties::Orange(prop)) => {
                            match prop {
                                OrangeProperty::StJamesPlace { state }
                                | OrangeProperty::TennesseeAve { state }
                                | OrangeProperty::NewYorkAve { state } => {
    ...
    pub fn get_owner(&self, board: &Board) -> Option<PlayerRef> {
        ...
        for player in players.iter() {
            ...
            for property in properties.iter() {
                match self {
                    OrangeProperty::StJamesPlace { state } => {
                        if let Properties::ColoredProperty(ColoredProperties::Orange(prop)) =
                            property
                        {
                            if prop == self {
